function [pfOut1,  wt, pfOut1_mean, pfOut2_mean, rho1, pfOut2] = particle__filter_now_cpu_2(par, mu_rho, sig_rho,y, v, r, alp, nParticle)
...
tmp1 =  arrayfun(@(weight,rho_t_1) sum(weight*d_conditional_WJ(y(it+1), pfOut2(it, :)-mu_f, mu_g, rho_t_1, mu_f, rho_f, 1) ),wt(it-1,:),rho1(it,:));
...
N_eff(it) = gather(1 / (wt(it,:) * wt(it,:)'));
    ...
    pfOut2(it+1,:) = pfOut2(it, :)-mu_f;
                % pfOut2_mean(it) = pfOut2(it,:) * wt(it,:)';

end


pfOut2 = gather(pfOut2);
